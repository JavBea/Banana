...
# ========= 路径（两选一）=========
DL_PATH = r"csf1.npy"  # 二维 (H,W) 的 uint8 码值（示例）
...
# 速度范围与极坐标半径
VMIN, VMAX = 0.0, 1.0   # m/s（同时用于 ARD 解码中的 Nyquist 近似）
R_MAX_KM   = 230.0         # 最大径向（km）


def to_npy():
    arf_file_path = Path(r"C:\Users\Me\Desktop\雷达数据\20250228\CS\C20250228090622.CSF")
    # 假设 Header 为 1036 字节
    ...
    try:
        ...
        # 可视化
        np.save('csf1.npy', grid)
        plt.imshow(grid, cmap="turbo", origin='lower')
        plt.colorbar(label="Reflectivity (dBZ)")
        plt.title(f"CS Visualization - {arf_file_path.name}")
        plt.axis("off")
        plt.show()
